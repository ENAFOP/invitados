...
return  $newDate;
 ...
 function contarPersonas($dms)
	 {
	 	$res=true;
		$db = $dms->getDB();
		$consultar = "SELECT COUNT(*) FROM app_invitado;";
		//echo "Consultar: ".$consultar;
		$res1 = $db->getResultArray($consultar);
		return $res1[0]['COUNT(*)'];
	 }

class SeedDMS_View_VerPersonas extends SeedDMS_Bootstrap_Style 
{
 /**
 ...
 **/
	...
	{ /* {{{ */
		...
		$this->htmlStartPage("Lista de personas", "skin-blue sidebar-mini  sidebar-collapse");
		$this->containerStart();
		$this->mainHeader();
		$this->mainSideBar();
 ...
 $this->startBoxPrimary("Lista de personas");
$this->contentContainerStart();
//////INICIO MI CODIGO
?>
 <div class="box">
            <div class="box-header">
              <h3 class="box-title">Haga click en el nombre para acceder al perfil</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body no-padding">
              <table id="tablaEventos" class="table table-hover table-striped table-condensed">
               ...
               <tbody>
                	<?php
                	$numGrupos=contarPersonas($dms);
                	//////////////// DIBUJO TABLA
                	$consultar = "SELECT * FROM app_invitado;";
					//echo "Consultar: ".$consultar;
				  	$res1 = $db->getResultArray($consultar);
                	for($cont=0;$cont<$numGrupos;$cont++)
                	{
                		echo ' <tr>';
                		//1. nombre
                    $idPersona=$res1[$cont]['id'];
                   echo "<td><a href=\"/out/out.VerPersona.php?id=".$idPersona."\">".$res1[$cont]['nombre']."</a></td>";
                		          
                		  // 2. descripcion
                		  echo "<td>".$res1[$cont]['cargo']."</td>";

                       // 3. Entidad
                      echo "<td>".$res1[$cont]['entidad']."</td>";
                       echo '</tr>';
